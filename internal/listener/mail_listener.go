...
func MailLinter() {
	...
	// 实例化amqp
	amqp := mq.New(amqpConfig, consts.MoguEmail, consts.ExchangeDirect, consts.MoguEmail, 1, 1, true)
	time.Sleep(time.Second * 1)
	if err := consumer.New(amqp, nil, handler.MailHandler.MailConsumerHandler).Consumer(); err != nil {
